...
xy.fw <- read.tps("bi377.demo.borealis.v.fervidus.forewings.class.tps", 
...
landmark.plot(xy.fw, specimen.number = 7)
...
landmark.plot(xy.fw, links = fw.links, specimen.number = c(4,6,16,17,9,11))

hw.links <- matrix(c(1:5,2:6), ncol = 2, byrow = FALSE)

landmark.plot(xy.hw, links = hw.links, specimen.number = 1:4)
...
cat(gpa.fw$provenance$align.procrustes)
...
# to make them comparable to original XY coordinates.
